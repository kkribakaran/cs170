...
int test1(HashMap *htable, int n, int k, int w){
  ...
  if((*htable).get(k)==-1)
      errorflag=1;
  if(errorflag==1)
    std::cout << "Failed to find  some elements"<<std::endl;
  return errorflag;
}
...
int test_get(HashMap *htable, int n, int k, int w){
  ...
  if(err)
    std::cout << "Failed to find  some elements or value was incorrect"<<std::endl;
  return err;
}
//
//put-only workload
...
int test_put_get(HashMap *htable, int n, int k, int w){
  ...
  err |= (*htable).get(k) != 0;
  if (err) std::cout << "get(k) after put("<<k<<",0)) = " << (*htable).get(k) << std::endl;
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).remove(i);
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).put(i,1);
  err |= (*htable).get(k) != 1;
  if (err) std::cout << "get(k) after put("<<k<<",1)) = " << (*htable).get(k) << std::endl;
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).remove(i);
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).put(i,i+1);
  err |= (*htable).get(k) != k+1;
  if (err) std::cout << "get(k) after put("<<k<<",i+1)) = " << (*htable).get(k) << std::endl;
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).remove(i);
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).put(i,k+3);
  err |= (*htable).get(k) != k+3;
  if (err) std::cout << "get(k) after put("<<k<<",k+3)) = " << (*htable).get(k) << std::endl;
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).remove(i);
  for (i=k-w; i<k+w+1; i++)
    if(i>=0) (*htable).put(i,i);
  err |= (*htable).get(k) != k;
  if (err) std::cout << "get(k) after put("<<k<<",i)) = " << (*htable).get(k) << std::endl;
  if(err)
    std::cout << "Failed to find  some elements or value was incorrect"<<std::endl;
  return err;
}
...
void *tfunc1(void *arg){
  ...
  for (i= b *k; i< b*k+b && i<NumKeys; i++){
     //std::cout << "Thread " <<k <<" does test1 "<<i<<std::endl;
...
void test(){
  ...
  std::cout << "Running tests in " << NumberofThread << " threads" << std::endl;
  ...
  for(j=0; j<NumIterations && !err; ++j) {
    if(NumberofThread>1){
      ...
      std::cout << "Finished iteration " << j << std::endl;
    } else {
      i=0;
      retval = tfunc ((void *)  i);
    } 
...
/*
    while (1)
    {
        ...
        std::cout<<"4. Repeat performance test"<<std::endl;
